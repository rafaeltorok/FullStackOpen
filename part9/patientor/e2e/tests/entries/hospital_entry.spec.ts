...
test.describe("Invalid Hospital entries", () => {
  ...
  test("missing both of the discharge fields", async ({ page }) => {
    ...
    await addHospitalEntry(page, otherFields);
    await expect(page.getByRole('alert')).toHaveText("The discharge field is required");
    await expect(page.locator(".patient-entry")).toHaveCount(initialEntriesLength);
  });
  ...
  test("missing the date of the discharge field", async ({ page }) => {
    ...
    await expect(page.getByRole('alert')).toHaveText("Missing one of the discharge fields");
    await expect(page.locator(".patient-entry")).toHaveCount(initialEntriesLength);
  });
