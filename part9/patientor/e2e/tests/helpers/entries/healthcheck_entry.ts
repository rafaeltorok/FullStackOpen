...
export async function assertHealthCheckEntry(entry: Locator, data: HealthCheckEntryInput) {
  ...
  await expect(entry.getByTestId('FavoriteIcon')).toHaveCSS('color', getHealthRatingColor(String(data.healthCheckRating)));
