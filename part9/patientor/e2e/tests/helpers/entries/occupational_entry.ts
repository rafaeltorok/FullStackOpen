...
export async function addOccupationalEntry(page: Page, data: OccupationalEntryInput) {
  ...
  if (data.employerName) {
    await page.getByRole('textbox', { name: 'Employer name' }).fill(String(data.employerName));
  }
  ...
  if (data.sickLeave?.startDate) {
    ...
    await startDate.getByRole('spinbutton', { name: 'Year' }).fill(String(data.sickLeave.startDate.year));
    await startDate.getByRole('spinbutton', { name: 'Month' }).fill(String(data.sickLeave.startDate.month));
    await startDate.getByRole('spinbutton', { name: 'Day' }).fill(String(data.sickLeave.startDate.day));
  }
  if (data.sickLeave?.endDate) {
    const startDate = page.getByRole('group', { name: 'End date' });
    await startDate.getByRole('spinbutton', { name: 'Year' }).fill(String(data.sickLeave.endDate.year));
    await startDate.getByRole('spinbutton', { name: 'Month' }).fill(String(data.sickLeave.endDate.month));
    await startDate.getByRole('spinbutton', { name: 'Day' }).fill(String(data.sickLeave.endDate.day));
  }
  
  await page.getByRole('button', { name: 'Add', exact: true }).click();
}

export async function assertOccupationalEntry(entry: Locator, data: OccupationalEntryInput) {
